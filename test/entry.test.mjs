...
test("a GM gets their own button, with no limit (D29)", () => {
  const gm = entryState({ state: "none", isGM: true, limit: 1, created: 5 });
  assert.deepEqual([gm.visible, gm.kind, gm.hidden], [true, "create", null], "the limit is for players");
  assert.equal(gm.tooltip, "CHARCREATOR.Entry.create.TooltipGM");
  assert.equal(entryState({ state: "editable", isGM: true }).kind, "continue", "their own draft carries on");
  assert.equal(gm.queue, null, "nothing waiting, no second button");
});

test("it's hidden at the character limit", () => {
...
test("the prompt nudges a player without a character, and always reports a result", () => {
  ...
  assert.equal(loginPrompt(entryState({ state: "none", isGM: true }), { isGM: true }), null,
    "a GM isn't nudged to make a character");
  const queued = loginPrompt(entryState({ state: "none", isGM: true, waiting: 2 }), { isGM: true });
  assert.deepEqual([queued.kind, queued.message], ["queue", "CHARCREATOR.Entry.queue.Prompt"], "only told what's waiting");
});
...
test("a GM also sees what is waiting to be created, as a second button", () => {
  assert.equal(entryState({ state: "none", isGM: true, waiting: 0 }).queue, null);
  const { queue } = entryState({ state: "none", isGM: true, waiting: 3 });
  assert.deepEqual([queue.kind, queue.count], ["queue", 3]);
  assert.equal(queue.label, "CHARCREATOR.Entry.queue.Button");
  const both = entryState({ state: "editable", isGM: true, waiting: 2 });
  assert.deepEqual([both.kind, both.queue.count], ["continue", 2], "their own draft and the queue, side by side");
  assert.equal(entryState({ state: "none", waiting: 3 }).queue, null, "players never see the queue");
});
