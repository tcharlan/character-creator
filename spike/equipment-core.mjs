/*
 * SPIKE 1.6 — throwaway. Pure logic (no Foundry globals) so it runs under `npm test` too.
 *
 * Interprets dnd5e's `system.startingEquipment` (a flat list of EquipmentEntryData linked by `group`)
 * plus `system.wealth`, and resolves a player's selection into items and currency. The rules content
 * comes from the item data; this only reads its structure (dnd5e has no helper for it — RESEARCH.md).
 *
 * Entry types (dnd5e 5.3.3 EquipmentEntryData): AND, OR (grouping); linked (key = item UUID);
 * weapon / armor / tool / focus (key = category key, choose `count` items); currency (key, count = amount).
 *
 * Selection (what the recipe stores for one class or background):
 *   { mode: "items" | "wealth",
 *     choices: { [orEntryId]: chosenChildEntryId },
 *     picks:   { [categoryEntryId]: [uuid, ...] },      // one UUID per unit of `count`
 *     wealth:  { total }                               // "wealth" mode with a dice formula (rolled, D15)
 *   }
 */

export const GROUPING = ["AND", "OR"];
export const CATEGORIES = ["weapon", "armor", "tool", "focus"];
export const KNOWN_TYPES = [...GROUPING, ...CATEGORIES, "linked", "currency"];

export const EQUIPMENT_ERRORS = {
  UNKNOWN_ENTRY_TYPE: "UNKNOWN_ENTRY_TYPE",
  MISSING_CHOICE: "MISSING_CHOICE",
  INVALID_CHOICE: "INVALID_CHOICE",
  WRONG_PICK_COUNT: "WRONG_PICK_COUNT",
  NOT_IN_CATEGORY: "NOT_IN_CATEGORY",
  NOT_PROFICIENT: "NOT_PROFICIENT",
  UNEXPECTED_SELECTION: "UNEXPECTED_SELECTION",
  BAD_MODE: "BAD_MODE",
  NO_WEALTH_OPTION: "NO_WEALTH_OPTION",
  WEALTH_OUT_OF_RANGE: "WEALTH_OUT_OF_RANGE"
};
const E = EQUIPMENT_ERRORS;

/** Build a tree from the flat entry list. Top-level entries have an empty or null `group`. */
export function buildTree(entries = []) {
  const nodes = new Map(entries.map(e => [e._id, { ...e, children: [] }]));
  const roots = [];
  for ( const node of nodes.values() ) {
    const parent = node.group ? nodes.get(node.group) : null;
    (parent ? parent.children : roots).push(node);
  }
  const bySort = (a, b) => (a.sort ?? 0) - (b.sort ?? 0);
  for ( const node of nodes.values() ) node.children.sort(bySort);
  roots.sort(bySort);
  return { _id: "__root", type: "AND", children: roots, nodes };
}

/** Amount of units an entry grants or asks for (dnd5e: empty count means 1). */
export const unitCount = node => (Number.isInteger(node.count) && node.count > 0 ? node.count : 1);

/**
 * Wealth offered by the item: `{ fixed }` for a plain number (2024: "or 155 GP"), `{ formula, min, max }`
 * for a simple dice formula "XdY" or "XdY * Z" (2014 rolled wealth), `{ formula }` if unrecognised, or null.
 */
export function wealthOption(wealth) {
  const w = String(wealth ?? "").trim();
  if ( !w ) return null;
  if ( /^\d+$/.test(w) ) return { fixed: Number(w) };
  const m = w.match(/^(\d+)d(\d+)\s*(?:[*x×]\s*(\d+))?$/i);
  if ( m ) {
    const [n, faces, mult] = [Number(m[1]), Number(m[2]), Number(m[3] ?? 1)];
    return { formula: w, min: n * mult, max: n * faces * mult };
  }
  return { formula: w };
}

/**
 * The decisions a player must make, given the choices made so far (only reachable branches).
 * @returns {{ id, kind: "or"|"category", options?: string[], type?, key?, count? }[]}
 */
export function listDecisions(tree, choices = {}) {
  const out = [];
  const walk = node => {
    if ( node.type === "OR" ) {
      out.push({ id: node._id, kind: "or", options: node.children.map(c => c._id) });
      const chosen = node.children.find(c => c._id === choices[node._id]);
      if ( chosen ) walk(chosen);
    } else if ( node.type === "AND" ) {
      node.children.forEach(walk);
    } else if ( CATEGORIES.includes(node.type) ) {
      out.push({ id: node._id, kind: "category", type: node.type, key: node.key, count: unitCount(node) });
    }
  };
  walk(tree);
  return out;
}

/**
 * Resolve a selection into items and currency, validating it.
 * @param {object} tree                  From buildTree().
 * @param {string|null} wealth           The item's `system.wealth`.
 * @param {object} selection             See the header.
 * @param {object} ctx
 * @param {(node, uuid) => boolean} ctx.inCategory    Is this UUID an allowed member of the category entry?
 * @param {(uuid) => boolean} ctx.isProficient        For `requiresProficiency` linked entries.
 * @returns {{ items: {uuid, count, entryId}[], currency: Record<string, number>, errors: {code, entryId?, detail?}[] }}
 */
export function resolveSelection(tree, wealth, selection = {}, ctx = {}) {
  const items = [];
  const currency = {};
  const errors = [];
  const fail = (code, entryId, detail) => errors.push({ code, entryId, detail });
  const mode = selection.mode ?? "items";

  // Unknown entry types anywhere in the data block the step (never silently skipped).
  for ( const node of tree.nodes.values() ) {
    if ( !KNOWN_TYPES.includes(node.type) ) fail(E.UNKNOWN_ENTRY_TYPE, node._id, node.type);
  }

  if ( mode === "wealth" ) {
    const option = wealthOption(wealth);
    if ( !option ) fail(E.NO_WEALTH_OPTION);
    else if ( "fixed" in option ) currency.gp = option.fixed;
    else {
      const total = Number(selection.wealth?.total);
      const ok = Number.isInteger(total) && (option.min === undefined || (total >= option.min && total <= option.max));
      if ( !ok ) fail(E.WEALTH_OUT_OF_RANGE, null, { total: selection.wealth?.total, ...option });
      else currency.gp = total;
    }
    if ( Object.keys(selection.choices ?? {}).length || Object.keys(selection.picks ?? {}).length ) {
      fail(E.UNEXPECTED_SELECTION, null, "choices/picks given in wealth mode");
    }
    return { items, currency, errors };
  }
  if ( mode !== "items" ) {
    fail(E.BAD_MODE, null, mode);
    return { items, currency, errors };
  }

  const reached = new Set();
  const walk = node => {
    reached.add(node._id);
    switch ( node.type ) {
      case "AND":
        node.children.forEach(walk);
        break;
      case "OR": {
        const choiceId = selection.choices?.[node._id];
        if ( !choiceId ) return fail(E.MISSING_CHOICE, node._id);
        const chosen = node.children.find(c => c._id === choiceId);
        if ( !chosen ) return fail(E.INVALID_CHOICE, node._id, choiceId);
        walk(chosen);
        break;
      }
      case "linked":
        if ( node.requiresProficiency && ctx.isProficient && !ctx.isProficient(node.key) ) {
          return fail(E.NOT_PROFICIENT, node._id, node.key);
        }
        items.push({ uuid: node.key, count: unitCount(node), entryId: node._id });
        break;
      case "currency":
        currency[node.key] = (currency[node.key] ?? 0) + unitCount(node);
        break;
      default:
        if ( CATEGORIES.includes(node.type) ) {
          const picks = selection.picks?.[node._id] ?? [];
          if ( picks.length !== unitCount(node) ) {
            return fail(E.WRONG_PICK_COUNT, node._id, { picks: picks.length, needed: unitCount(node) });
          }
          const bad = picks.filter(u => !ctx.inCategory?.(node, u));
          if ( bad.length ) return fail(E.NOT_IN_CATEGORY, node._id, bad);
          for ( const uuid of picks ) items.push({ uuid, count: 1, entryId: node._id });
        }
    }
  };
  walk(tree);

  // Choices or picks for entries that aren't reached (unchosen branches, made-up ids) are tampering.
  for ( const id of [...Object.keys(selection.choices ?? {}), ...Object.keys(selection.picks ?? {})] ) {
    if ( !reached.has(id) ) fail(E.UNEXPECTED_SELECTION, id);
  }
  return { items: mergeItems(items), currency, errors };
}

/** Merge identical UUIDs (e.g. two picks of the same weapon) into one line with a summed count. */
function mergeItems(items) {
  const byUuid = new Map();
  for ( const it of items ) {
    const prev = byUuid.get(it.uuid);
    if ( prev ) prev.count += it.count;
    else byUuid.set(it.uuid, { ...it });
  }
  return [...byUuid.values()];
}

/**
 * A default selection: first option of every OR (or the first whose linked items pass `ctx.isProficient`),
 * the first `count` candidates of every category. Used by tests, and a model for auto-select (D4).
 * @param {(node) => string[]} candidates   Allowed UUIDs for a category entry.
 */
export function defaultSelection(tree, { candidates = () => [], isProficient } = {}) {
  const selection = { mode: "items", choices: {}, picks: {} };
  const usable = node => {
    if ( node.type === "linked" ) return !node.requiresProficiency || !isProficient || isProficient(node.key);
    if ( CATEGORIES.includes(node.type) ) return candidates(node).length >= 1;
    return node.children?.every?.(c => node.type !== "AND" || usable(c)) ?? true;
  };
  const walk = node => {
    if ( node.type === "OR" ) {
      const chosen = node.children.find(usable) ?? node.children[0];
      if ( !chosen ) return;
      selection.choices[node._id] = chosen._id;
      walk(chosen);
    } else if ( node.type === "AND" ) node.children.forEach(walk);
    else if ( CATEGORIES.includes(node.type) ) {
      const c = candidates(node);
      selection.picks[node._id] = Array.from({ length: unitCount(node) }, (_, i) => c[i % Math.max(c.length, 1)]).filter(Boolean);
    }
  };
  walk(tree);
  return selection;
}
